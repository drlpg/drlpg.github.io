...
// 移动端头像点击切换功能
(() => {
  const initMobileAvatarToggle = () => {
    if (window.innerWidth > 768) return;

    const cardInfo = document.querySelector(".card-info");
    if (!cardInfo) return;

    const avatarImg = cardInfo.querySelector(".avatar-img");
    const avatarDesc = cardInfo.querySelector(".avatar-description");

    if (!avatarImg || !avatarDesc) return;

    const toggleAvatar = (e) => {
      e.stopPropagation();
      cardInfo.classList.toggle("avatar-clicked");
    };

    avatarImg.addEventListener("click", toggleAvatar);
    avatarDesc.addEventListener("click", toggleAvatar);

    // 点击其他区域关闭
    document.addEventListener("click", (e) => {
      if (!cardInfo.contains(e.target)) {
        cardInfo.classList.remove("avatar-clicked");
      }
    });
  };

  // 页面加载时初始化
  document.addEventListener("DOMContentLoaded", initMobileAvatarToggle);

  // 如果使用了pjax，在pjax完成后重新初始化
  if (typeof btf !== "undefined") {
    btf.addGlobalFn(
      "pjaxComplete",
      initMobileAvatarToggle,
      "mobileAvatarToggle"
    );
  }
})();
...
(() => {
  ...
  const fixSidebarLayout = () => {
    ...
    // 强制将头像包装器移到最前面
    const avatarWrapper = sidebarMenus.querySelector(".sidebar-avatar-wrapper");
    if (avatarWrapper && avatarWrapper.parentElement === sidebarMenus) {
      sidebarMenus.insertBefore(avatarWrapper, sidebarMenus.firstChild);
    }
  ...
  const observeSidebar = () => {
    ...
    sidebarObserver.observe(sidebarMenus, {
      attributes: true,
