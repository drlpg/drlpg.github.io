document.addEventListener('DOMContentLoaded', () => {
   ...
   * 处理下拉箭头与导航栏重叠的问题
   */
  const handleScrollDownOverlap = () => {
    const $scrollDownEle = document.getElementById('scroll-down')
    const $header = document.getElementById('page-header')
    
    if (!$scrollDownEle || !$header) return

    const checkOverlap = () => {
      const currentTop = window.scrollY || document.documentElement.scrollTop
      
      // 当在最顶部时（scrollTop = 0），显示下拉箭头
      if (currentTop === 0) {
        $scrollDownEle.style.opacity = '1'
        $scrollDownEle.style.pointerEvents = 'auto'
      }
      // 当导航栏可见时（向上滚动时显示的导航栏），隐藏下拉箭头
      else if ($header.classList.contains('nav-visible')) {
        $scrollDownEle.style.opacity = '0'
        $scrollDownEle.style.pointerEvents = 'none'
      }
      // 其他情况（向下滚动时导航栏隐藏），显示下拉箭头
      else {
        $scrollDownEle.style.opacity = '1'
        $scrollDownEle.style.pointerEvents = 'auto'
      }
    ...
    // 监听滚动事件
    const scrollHandler = btf.throttle(checkOverlap, 100)
  ...
  const addHighlightTool = () => {
    ...
    const alertInfo = (ele, text) => {
      // 使用现有的顶部提示栏函数，并修改消息为"复制成功，可粘贴"
      if (typeof window.showCopySuccessNotification === 'function') {
        window.showCopySuccessNotification('复制成功，可粘贴');
      } else if (GLOBAL_CONFIG.Snackbar !== undefined) {
 ...
 */
  ...
  const refreshFn = () => {
    ...
    GLOBAL_CONFIG_SITE.isHome && handleScrollDownOverlap()
    scrollFn()
...
// 分类标签容器点击事件
const initCategoryClick = () => {
  // 主页面分类
  const categoryItems = document.querySelectorAll('.category-lists li');
  ...
  // 侧边栏分类
  const asideCategoryItems = document.querySelectorAll('.card-categories .card-category-list-item');
  
  asideCategoryItems.forEach(function(item) {
    // 添加点击样式
    item.style.cursor = 'pointer';
    ...
    item.addEventListener('click', function(e) {
      ...
      if (link) {
        // 如果点击的不是链接本身，则触发链接跳转
        if (e.target !== link && !e.target.closest('i')) {
          window.location.href = link.href;
        }
      }
    });
...
// 标签页面容器点击事件 - 确保整个标签区域可点击
const initTagClick = () => {
  // 等待标签内容加载完成
  setTimeout(() => {
    const tagItems = document.querySelectorAll('.tag-cloud-list a');
