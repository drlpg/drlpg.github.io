...
function fixMusicPlayerAutoCollapse() {
  ...
  if (window.musicPlayerManager) {
    window.musicPlayerManager.onReady((aplayer, metingElement) => {
      setupPlayerAutoCollapse(aplayer, metingElement);
    });
    return;
  }
  ...
  setupPlayerAutoCollapse(metingElement.aplayer, metingElement);
}

// 设置播放器自动收缩功能
function setupPlayerAutoCollapse(aplayer, metingElement) {
  const musicElement = document.querySelector('#nav-music');
