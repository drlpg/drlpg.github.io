// 音乐播放器自动跳过脚本
const SKIP_CONFIG = window.MUSIC_SKIP_CONFIG || {
  loadTimeout: 10000,
  playTimeout: 5000,
...
function handlePlaybackError(aplayer, errorType, errorMsg) {
  ...
  if (currentSongState.retryCount < SKIP_CONFIG.maxRetries) {
    ...
    if (SKIP_CONFIG.debug) {
      console.log(`🔄 重试播放 (${currentSongState.retryCount}/${SKIP_CONFIG.maxRetries}):`, currentSongState.name);
    }
  ...
  if (SKIP_CONFIG.debug) {
    console.log('⏭️ 跳过无法播放的歌曲:', currentSongState.name);
  }
