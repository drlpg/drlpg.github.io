// 音乐封面颜色提取器
// 避免重复声明
if (typeof window.CoverColorExtractor === 'undefined') {
window.CoverColorExtractor = class CoverColorExtractor {
  constructor() {
...
function applyMusicColor(color) {
  ...
  document.documentElement.style.setProperty('--music-dynamic-border', borderColor);
  
  // 设置阴影颜色（20%透明度）
  const shadowColor = `${darkenedColor}33`; // 33 = 20% opacity in hex
  document.documentElement.style.setProperty('--music-dynamic-shadow', shadowColor);
...
function initSimpleMusicColor() {
  ...
  if (window.musicPlayerManager) {
    window.musicPlayerManager.onReady((aplayer, metingElement) => {
      setupMusicColorExtraction(aplayer, metingElement);
    });
    return;
  }
  ...
  setupMusicColorExtraction(metingElement.aplayer, metingElement);
}

// 设置音乐颜色提取功能
function setupMusicColorExtraction(aplayer, metingElement) {
  // 初始化歌曲跟踪变量
  window.lastProcessedSong = null;
  
  // 立即为当前歌曲提取颜色
  extractCurrentSongColor(aplayer).then(color => {
    if (color) {
      applyMusicColor(color);
      // 记录当前处理的歌曲
      const currentMusic = aplayer.list && aplayer.list.audios 
        ? aplayer.list.audios[aplayer.list.index || 0] 
        : null;
      if (currentMusic && currentMusic.pic) {
  ...
  try {
    ...
    songChangeEvents.forEach(eventName => {
      aplayer.on(eventName, async function() {
        // 延迟处理，避免干扰播放器初始化
        setTimeout(async () => {
    ...
    setInterval(() => {
      try {
        ...
        if (currentMusic && currentMusic.pic && window.lastProcessedSong !== currentMusic.pic) {
          ...
          setTimeout(async () => {
            try {
              const color = await extractCurrentSongColor(aplayer);
              if (color) {
                applyMusicColor(color);
              }
            } catch (error) {
