...
(function () {
  ...
  function init() {
    ...
    setTimeout(() => {
      if (loadedCount + errorCount < totalImages) {
        console.error(
          `哔哩哔哩背景: 加载超时，已加载 ${loadedCount}/${totalImages}`
        );
        checkAllLoaded();
      }
    }, 10000);
    ...
    resizeHandler = function () {
      ...
      if (isMobile) {
        // 移动端：支持高DPI，但限制最大值为1.5倍
        if (hasLowMemory || isVeryLowResolution) {
          dpr = 1;
        } else {
      ...
      if (isMobile) {
        ...
        display.style.width = scaledWidth + "px";
        display.style.height = scaledHeight + "px";
        display.style.left = (headerWidth - scaledWidth) / 2 + "px";
        display.style.top = (headerHeight - scaledHeight) / 2 + "px";
        display.style.pointerEvents = "none";
        display.style.transform = "translate3d(0, 0, 0)";
        display.style.transition = "opacity 0.016s linear";
      } else {
        // 桌面端使用常规transform，同样添加性能优化
        display.style.position = "absolute";
    ...
    const deviceInfo = (() => {
      ...
      const isLowPerformance = isMobile || isLowResolution || hasLowMemory;
    ...
    // 根据设备性能调整频谱更新频率
    const updateInterval = deviceInfo.isMobile
      ? 200
      : deviceInfo.isLowResolutionDevice
      ? 150
      : 100;
    ...
    const frameSkipInterval = deviceInfo.isMobile
      ? 3
    ...
    function render() {
      ...
      if (deviceInfo.isMobile) {
        // 移动端：2K分辨率缩放绘制
        displayCtx.drawImage(cvs, 0, 0, 3960, 2160, 0, 0, 1980, 1080);
      } else {
        // 桌面端：4K分辨率1:1绘制
