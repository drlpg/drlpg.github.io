...
(function() {
  ...
  function isPostPage() {
    ...
    // 如果URL看起来像文章页面（不是分类、标签、关于等页面），先返回true
    const excludePaths = ['/categories', '/tags', '/archives', '/about', '/links', '/music', '/gallery'];
    ...
    if (!isExcludedPath) {
      ...
      const commentSelectors = [
        ...
        '.tk-comments-container', '.vcomments',
        '.twikoo', '.valine', '.comment'
      ];
      
      const postContentSelectors = [
        '.post-content', '.article-content', '#post',
        '.article-entry', '.post-body', '.content',
        'main', '.main', '#main'
      ];
      
      const hasComments = commentSelectors.some(selector => document.querySelector(selector));
      const hasPostContent = postContentSelectors.some(selector => document.querySelector(selector));
      
      // 只要有其中一个就认为是文章页面（降低判断门槛）
      return hasComments || hasPostContent;
    }
  ...
  function checkAndClosePopup() {
    ...
    if (!isPost && (isPopupVisible || isTimerRunning)) {
      stopHotCommentTimer();
    }
  }
  ...
  function init() {
    ...
    // 等待页面完全加载后启动（仅在启用时且在文章页面）
    const startTimer = () => {
      if (isHotCommentEnabled && isPostPage()) {
    ...
    const checkUrlChange = () => {
      ...
      if (currentUrl !== lastUrl) {
        ...
        setTimeout(() => {
          if (isPostPage() && isHotCommentEnabled && !isTimerRunning) {
            startHotCommentTimer();
          }
        }, 2000); // 增加延迟到2秒，确保页面内容完全加载
      }
    };
