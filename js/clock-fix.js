...
window.clockDebug = {
  ...
  getStatus: function() {
    return {
      ...
      isOnline: window.clockNetworkManager ? window.clockNetworkManager.isOnline() : navigator.onLine,
      retryCount: window.clockNetworkManager ? window.clockNetworkManager.retryCount : 0
    }
  },
  
  // 强制重新初始化
  forceReinit: function() {
    if (window.clockNetworkManager) {
      window.clockNetworkManager.retryCount = 0;
    }
    if (typeof window.reinitializeClock === 'function') {
      window.reinitializeClock()
    }
