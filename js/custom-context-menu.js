...
(function() {
  ...
  function generateDefaultMenuContent() {
    // 获取热评弹窗状态
    const isHotCommentEnabled = typeof window.getHotCommentStatus === 'function' ? window.getHotCommentStatus() : (localStorage.getItem('hot-comment-enabled') !== 'false');
  ...
  if (typeof btf !== 'undefined' && btf.addGlobalFn) {
    // 页面切换前清理
    btf.addGlobalFn('pjaxSend', function() {
      // 隐藏可能存在的右键菜单
      hideAllContextMenus();
