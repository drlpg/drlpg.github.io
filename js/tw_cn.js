document.addEventListener('DOMContentLoaded', () => {
  ...
  targetEncoding = Number(btf.saveToLocal.get(targetEncodingCookie)) || defaultEncoding
  ...
  const translateBody = (wasSimplified = false, fobj) => {
    ...
    for (const node of nodes) {
      ...
      if (node.nodeType === Node.ELEMENT_NODE) {
        ...
        // Translate title, alt, placeholder based on previous state
        if (title) {
          node.title = wasSimplified ? Traditionalized(title) : Simplized(title)
        }
        if (alt) {
          node.alt = wasSimplified ? Traditionalized(alt) : Simplized(alt)
        }
        if (placeholder) {
          node.placeholder = wasSimplified ? Traditionalized(placeholder) : Simplized(placeholder)
        }

        // Translate input value except text and hidden types
        if (tagName === 'INPUT' && value && type !== 'text' && type !== 'hidden') {
          node.value = wasSimplified ? Traditionalized(value) : Simplized(value)
        }
        ...
        node.data = wasSimplified ? Traditionalized(node.data) : Simplized(node.data)
      }
    }
  }
