...
(function() {
  ...
  const lazyFeatures = {
    ...
    music: {
      scripts: [
        '/js/music-color-simple.js',
        '/js/music-player-auto-collapse.js',
        '/js/music-auto-skip.js',
        '/js/music-context-menu.js',
        '/js/music-skip-config.js'
      ],
      trigger: () => document.querySelector('#nav-music') || document.querySelector('.aplayer')
    },

    // 右键菜单功能
    contextMenu: {
      scripts: [
        '/js/custom-context-menu.js',
        '/js/mobile-context-menu-disable.js'
      ],
    ...
    share: {
      scripts: ['/js/share.js'],
      trigger: () => document.querySelector('.share-button') || document.querySelector('[data-share]')
    },

    // 复制功能
    clipboard: {
      scripts: ['/js/clipboard.js'],
      trigger: () => document.querySelector('.copy-btn') || document.querySelector('[data-clipboard]')
    },
    ...
    hotComment: {
      ...
      trigger: () => {
        ...
        const commentSelectors = [
          '.comment-hot', '#post-comment', '.comments', '#comments',
          '.tk-comments-container', '.vcomments', '.twikoo', '.valine',
          '.comment', '.gitalk-container', '.utterances', '.disqus-thread',
          '.artalk', '.waline', '.giscus'
        ];
    ...
    grayscale: {
      ...
      trigger: () => document.querySelector('#grayscale-toggle') || document.querySelector('[data-grayscale]')
    }
  };
    ...
    const observer = new IntersectionObserver((entries) => {
      entries.forEach(entry => {
        if (entry.isIntersecting) {
          ...
          // 检查评论区域
          if (element.matches('.comment-hot, #post-comment, .comments, #comments, .tk-comments-container, .vcomments, .twikoo, .valine, .comment, .gitalk-container, .utterances, .disqus-thread, .artalk, .waline, .giscus')) {
